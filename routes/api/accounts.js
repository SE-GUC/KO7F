...
router.post("/CreateAccount", (req, res) => {
  ...
  if (!Account_id || typeof Account_id !== "number")
    return res
      .status(404)
      .send({ err: "You must enter the Account ID and as an integer" });
  if (!username)
    return res.status(404).send({ err: "You must enter a username" });
  if (!password)
    return res.status(404).send({ err: "You must enter password" });
  if (!age) return res.status(404).send({ err: "You must enter your age" });
  if (!major) return res.status(404).send({ err: "You must enter your major" });
...
router.put("/Updateaccount/: Account_id,", (req, res) => {
  Account;
  const isEntered = AccountArr.some(
    Account => Account.Account_id === parseInt(req.params.Account_id)
  );
  if (isEntered) {
    ...
    AccountArr.forEach(Account => {
      if (Account.Account_id === parseInt(req.params.Account_id)) {
        ...
        Account.username = Accountusername.username
