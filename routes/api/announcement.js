...
router.post("/Announcement", (req, res) => {
  ...
  if (!announcement_id || typeof announcement_id !== "number")
    return res
      .status(404)
      .send({ err: "You must enter the Announcement ID and as an integer" });
  if (!title) return res.status(404).send({ err: "You must enter a title" });
...
router.put("/UpdateAnnouncement/:id", (req, res) => {
  const isEntered = AnnouncementArr.some(
    Event => Announcement.announcement_id === parseInt(req.params.id)
  );
  if (isEntered) {
    ...
    AnnouncementArr.forEach(Announcement => {
      if (Announcement.announcement_id === parseInt(req.params.id)) {
        Announcement.title = announcementUpdated.title
          ? announcementUpdated.name
          : Announcement.name;
        res.json({
          msg: "The announcement is updated successfully",
          Announcement
        });
      }
    });
